...
b32 read_player_profiles_from_disc(Player_Profile *profiles) {
    ...
    if (file != INVALID_HANDLE_VALUE) {
        ...
        printf("%s() failed to load player profiles!\n", __FUNCTION__);
    }
...
void init_game(Game *game) {
    ...
    {   
        ...
        // bitmaps
        assert(load_bitmap("data\\bitmaps\\pacman.bmp", &resources->bitmaps.pacman));
        assert(load_bitmap("data\\bitmaps\\ghost_red.bmp", &resources->bitmaps.ghost_red));
        assert(load_bitmap("data\\bitmaps\\ghost_pink.bmp", &resources->bitmaps.ghost_pink));
        assert(load_bitmap("data\\bitmaps\\ghost_cyan.bmp", &resources->bitmaps.ghost_cyan));
        assert(load_bitmap("data\\bitmaps\\ghost_orange.bmp", &resources->bitmaps.ghost_orange));
        assert(load_bitmap("data\\bitmaps\\ghost_as_prey.bmp", &resources->bitmaps.ghost_as_prey));
        assert(load_bitmap("data\\bitmaps\\ghost_eyes.bmp", &resources->bitmaps.ghost_eye));
        assert(load_bitmap("data\\bitmaps\\ghost_pupils.bmp", &resources->bitmaps.ghost_pupil));
        assert(load_bitmap("data\\bitmaps\\dot_large.bmp", &resources->bitmaps.dot_large));
        assert(load_bitmap("data\\bitmaps\\dot_small.bmp", &resources->bitmaps.dot_small));
        assert(load_bitmap("data\\bitmaps\\background.bmp", &resources->bitmaps.background));
        assert(load_bitmap("data\\bitmaps\\pacman_atlas.bmp", &resources->bitmaps.pacman_atlas));
        

        char path[24];
        for (u32 index = 0; index < 16; ++index) {
            _snprintf_s(path, 24, _TRUNCATE, "data\\bitmaps\\wall%02u.bmp", index);
            assert(load_bitmap(path, &resources->bitmaps.walls[index]));
        }
    
        // fonts
        assert(load_font("data\\fonts", "font", &resources->font));
    
        // wavs
        assert(load_wav("data\\audio\\eat_large_dot.wav", &resources->wavs.eat_large_dot));
        assert(load_wav("data\\audio\\eat_small_dot.wav", &resources->wavs.eat_small_dot));
        assert(load_wav("data\\audio\\ghost_dies.wav", &resources->wavs.ghost_dies));
        assert(load_wav("data\\audio\\won.wav", &resources->wavs.won));
        assert(load_wav("data\\audio\\lost.wav", &resources->wavs.lost));
        assert(load_wav("data\\audio\\nope.wav", &resources->wavs.nope));
        ...
        assert(init_voices(&game->audio_state, &game->resources.wavs.eat_small_dot));
        ...
        // load worlds
        {
            ...
            do
            {
                ...
                assert(load_world_from_file(world, "data\\worlds", file_data.cFileName, &game->resources));
            }
